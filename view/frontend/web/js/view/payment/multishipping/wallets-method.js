define(
    [
        'Sapient_Worldpay/js/view/payment/method-renderer/wallets-method',
        'jquery',
        'domReady!'
    ],
    function (
        ...
        return Component.extend({
            ...
            onContinue: function (e) {
                ...
                if (this.getselectedCCType() == undefined) {
                    $('#wallet-type-error').css('display', 'block');
                    $('#wallet-type-error').html("<div>" + this.getCreditCardExceptions('CCAM6') + "</div>");
                    return false;
                }
                if ($form.validation() && $form.validation('isValid')) {
                    $('#wallet-type-error').css('display', 'none');
                    this.preparePayment();
                    return false;
                }
                return false;
            ...
            isWorldpayMethodSelected: function () {
                ...
                var worldpay = methods.filter(function (index, value) {
                    if (value.id == "p_method_worldpay_wallets")
                        return value;
                });
