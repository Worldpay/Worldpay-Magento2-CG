...
class SubscriptionOrders extends \Magento\Framework\View\Element\Template
{
    /**
     ...
     * @return mixed
     */
    public function getsubscriptionOrderCollection()
    {
        $subscriptionId = $this->getRequest()->getParam('subscription_id');
        $timeFilter = $this->getRequest()->getParam('timeFilter');
        $timeFilterby = $this->subscriptionOrder
            ->getFilterDate($timeFilter=null);
        $subscriptionOrderCollection = $this->subscriptionOrder
            ->getsubscriptionOrderCollection($subscriptionId, $timeFilterby);
     ...
     * Return cancel Url
     *
     * @param SubscriptionOrder $subscription
     * @return string
     */
    public function getCancelUrl(SubscriptionOrder $subscription)
    {
        return $this->getUrl(
            'worldpay/recurring_order/cancel/',
            [
                'id' => $subscription->getOriginalOrderId(),
                '_secure' => true
            ]
        );
    }
     ...
     * @return string
     */
    public function getViewlUrl(SubscriptionOrder $subscription)
    {
        return $this->getUrl(
            'sales/order/view',
            [
                'order_id' => $subscription->getOriginalOrderId(),
                '_secure' => true
            ]
        );
    }
    /**
     * Return Skip Orderl Url
     *
     * @return string
     */
    public function getSkipOrderlUrl()
    {
        $subscriptionId = $this->getRequest()->getParam('subscription_id');
        return $this->_urlBuilder->getUrl(
    ...
    protected function _prepareLayout()
    {
        ...
        $pagination = $this->getLayout()
            ...
            ->setCollection($this->getsubscriptionOrderCollection())
            ->setPath('worldpay/recurring_order/view');
